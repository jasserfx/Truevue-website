...
window.TVi18n = (function () {
  ...
  const T = {
    ...
    'home.hero.sub':     { en: 'We craft cinematic visuals that transform brands into experiences photography and video that make the world stop and stare. test', fr: 'Nous créons des visuels cinématiques qui transforment les marques en expériences photographie et vidéo qui captivent.' },
  ...
  function apply(lang) {
    ...
    document.querySelectorAll('.lang-toggle').forEach(btn => {
      btn.querySelectorAll('[data-lang]').forEach(opt => {
  ...
  return {
    apply,
    toggle: () => apply(currentLang === 'en' ? 'fr' : 'en'),
    get: (key) => T[key] ? (T[key][currentLang] || T[key]['en']) : key,
    current: () => currentLang,
    init: () => apply(currentLang),
  };
